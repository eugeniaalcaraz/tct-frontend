...
const rowStructure = [
    [
        { label: "Origen", data: "getCountryById(idCountry)" },
        { label: "Cantidad Total", data: "quantity" },
    ],
    [
        { label: "Proveedor", data: "getSupplierById(idSupplier)" },
        { label: "Código de Fábrica", data: "fabricCode" },
    ],
];
...
export const Shipment = () => {
    ...
    return (
        <section>
            ...
            {/* TODO telas.map((tela)=>) */}
            <Stack rowGap={"40px"}>
                {/* TODO telas.map((tela, index)=><ComboTable embarque={tela} combo={index}/>) */}
                {[...Array(3).keys()].map((index) => (
                    <ComboTable key={uuid()} combo={index + 1} />
                ))}
            </Stack>
        </section>
    );
